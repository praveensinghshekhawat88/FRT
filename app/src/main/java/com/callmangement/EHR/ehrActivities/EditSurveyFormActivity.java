...
public class EditSurveyFormActivity extends BaseActivity {

    Activity mActivity;
    private ActivityEditSurveyFormBinding binding;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        binding = ActivityEditSurveyFormBinding.inflate(getLayoutInflater());
    ...
    private void setUpData() {
        ...
        if (surveyFormDetailsInfo.getCustomerName() != null)
            binding.inputCustomerName.setText(surveyFormDetailsInfo.getCustomerName());

        if (surveyFormDetailsInfo.getBill_ChallanNo() != null)
            binding.inputBillChallanNo.setText(surveyFormDetailsInfo.getBill_ChallanNo());

        if (surveyFormDetailsInfo.getAddress() != null)
            binding.inputCustomerAddress.setText(surveyFormDetailsInfo.getAddress());

        if (surveyFormDetailsInfo.getBillRemark() != null)
            binding.inputBillRemarks.setText(surveyFormDetailsInfo.getBillRemark());

        if (surveyFormDetailsInfo.getPointOfContact() != null)
            binding.inputPointOfContact.setText(surveyFormDetailsInfo.getPointOfContact());

        if (surveyFormDetailsInfo.getMobileNumber() != null)
            binding.inputMobileNumber.setText(surveyFormDetailsInfo.getMobileNumber());
        ...
        if (surveyFormDetailsInfo != null && surveyFormDetailsInfo.getTypeOfCallId() != null) {
            if (surveyFormDetailsInfo.getTypeOfCallId().contains(",")) {
                ...
                binding.checkBoxSupply.setChecked(typeOfCallIdList.contains("1"));

                if (typeOfCallIdList.contains("2")) {
                    binding.checkBoxWarranty.setChecked(true);
                } else {
                    binding.checkBoxSupply.setChecked(false);
                }

                binding.checkBoxInstallation.setChecked(typeOfCallIdList.contains("3"));

                binding.checkBoxPayable.setChecked(typeOfCallIdList.contains("4"));

                binding.checkBoxCourtesy.setChecked(typeOfCallIdList.contains("5"));
            } else {
                if (surveyFormDetailsInfo.getTypeOfCallId().length() > 0) {

                    binding.checkBoxSupply.setChecked(surveyFormDetailsInfo.getTypeOfCallId().equalsIgnoreCase("1"));
                    ...
                    binding.checkBoxInstallation.setChecked(surveyFormDetailsInfo.getTypeOfCallId().equalsIgnoreCase("3"));

                    binding.checkBoxPayable.setChecked(surveyFormDetailsInfo.getTypeOfCallId().equalsIgnoreCase("4"));

                    binding.checkBoxCourtesy.setChecked(surveyFormDetailsInfo.getTypeOfCallId().equalsIgnoreCase("5"));
                } else {
                    binding.checkBoxSupply.setChecked(false);
                    binding.checkBoxSupply.setChecked(false);
                    binding.checkBoxInstallation.setChecked(false);
                    binding.checkBoxPayable.setChecked(false);
                    binding.checkBoxCourtesy.setChecked(false);
                }
            }
        }

        if (surveyFormDetailsInfo.getItemDetail() != null)
            binding.inputItemDetails.setText(surveyFormDetailsInfo.getItemDetail());

        if (surveyFormDetailsInfo.getPurchaseOrderDtl() != null)
            binding.inputPurchaseOrderDetails.setText(surveyFormDetailsInfo.getPurchaseOrderDtl());

        if (surveyFormDetailsInfo.getInstalledMachineSpecification() != null)
            binding.inputSpecificationOfMachineInstalled.setText(surveyFormDetailsInfo.getInstalledMachineSpecification());

        if (surveyFormDetailsInfo.getAccessesory() != null)
            binding.inputAnyAccessory.setText(surveyFormDetailsInfo.getAccessesory());

        if (surveyFormDetailsInfo.getInstallationDone() != null)
            binding.inputInstallationDone.setText(surveyFormDetailsInfo.getInstallationDone());

        if (surveyFormDetailsInfo.getCustomer_Remark() != null)
            binding.inputCustomerRemarks.setText(surveyFormDetailsInfo.getCustomer_Remark());

        if (surveyFormDetailsInfo.getEngineerName() != null)
            binding.inputEngineerName.setText(surveyFormDetailsInfo.getEngineerName());
    ...
    private void init() {
        ...
        binding.actionBar.textToolbarTitle.setText(getResources().getString(R.string.fill_edit_survey_form));
    ...
    private void setClickListener() {
        ...
        binding.framedateOfInstallation.setOnClickListener(view -> {
            ...
            DatePickerDialog datePickerDialog = new DatePickerDialog(mActivity, dateFromDate, myCalendarPODate
        ...
        binding.buttonFillNewSurveyForm.setOnClickListener(view -> {
            ...
            if (TextUtils.isEmpty(customer_name)) {
                ...
                editSurveyForm(USER_Id, surveyFormDetailsInfo.getServeyFormId(), surveyFormDetailsInfo.getTicketNo(),
    ...
    private void editSurveyForm(String UserID, String ServeyFormId, String TicketNo, String Bill_ChallanNo, String Address, String PointOfContact, String BillRemark,
        ...
        if (Utils.isNetworkAvailable(mActivity)) {
            ...
            String paramStr = EditSurveyFormQueryParams();
            ...
            paramData.put(splitArray[3], Bill_ChallanNo);
            paramData.put(splitArray[4], Address);
            paramData.put(splitArray[5], PointOfContact);
            paramData.put(splitArray[6], BillRemark);
            paramData.put(splitArray[7], MobileNumber);
            paramData.put(splitArray[8], InstallationDateStr);
            paramData.put(splitArray[9], TypeOfCallId);
            paramData.put(splitArray[10], ItemDetail);
            paramData.put(splitArray[11], PurchaseOrderDtl);
            paramData.put(splitArray[12], InstalledMachineSpecification);
            paramData.put(splitArray[13], Accessesory);
            paramData.put(splitArray[14], InstallationDone);
            paramData.put(splitArray[15], Customer_Remark);
            paramData.put(splitArray[16], CustomerName);
            paramData.put(splitArray[17], EngineerName);

            Call<ModelEditASurveyForm> call = apiInterface.callEditSurveyFormApi(EditSurveyForm(), paramData);
            call.enqueue(new Callback<ModelEditASurveyForm>() {
                @Override
                public void onResponse(@NonNull Call<ModelEditASurveyForm> call, @NonNull Response<ModelEditASurveyForm> response) {
                ...
                @Override
                public void onFailure(@NonNull Call<ModelEditASurveyForm> call, @NonNull Throwable error) {
                    Utils.hideCustomProgressDialogCommonForAll();
