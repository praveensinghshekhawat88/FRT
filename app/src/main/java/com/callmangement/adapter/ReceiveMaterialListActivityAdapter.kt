...
class ReceiveMaterialListActivityAdapter(
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        val separator = model.dispatchDateStr!!.split(" ".toRegex()).dropLastWhile { it.isEmpty() }
                .toTypedArray()
        ...
        holder.binding.tvSenderName.text = "Send by " + model.dispatcherName
        holder.binding.textRemarkStr.text =
            model.dispatcherName + " " + context.resources.getString(R.string.remark)
        ...
        holder.binding.buttonReceive.setOnClickListener { view: View? ->
            val intent = Intent(
                context,
                ReceiveMaterialListDetailsActivity::class.java
            )
            intent.putExtra("invoice_id", model.invoiceId)
            intent.putExtra("dispatch_id", model.dispatchId)
    ...
    class ViewHolder(val binding: ItemReceiveMaterialListBinding) :
