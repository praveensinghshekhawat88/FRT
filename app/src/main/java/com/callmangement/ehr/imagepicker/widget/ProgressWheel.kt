...
class ProgressWheel : View {
    ...
    private fun setAnimationEnabled() {
        ...
        val animationValue = if (currentApiVersion >= Build.VERSION_CODES.JELLY_BEAN_MR1) {
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        if (isSpinning) {
            ...
            if (!linearProgress) {
                val factor = 2.0f
                offset =
                    (1.0f - (1.0f - mProgress / 360.0f).toDouble().pow((2.0f * factor).toDouble())).toFloat() * 360.0f
                progress =
                    (1.0f - (1.0f - mProgress / 360.0f).toDouble().pow(factor.toDouble())).toFloat() * 360.0f
            }
    ...
    private fun updateBarLength(deltaTimeInMilliSeconds: Long) {
        if (pausedTimeWithoutGrowing >= pauseGrowingTime) {
            ...
            val distance =
                cos((timeStartGrowing / barSpinCycleTime + 1) * Math.PI) as Float / 2 + 0.5f
    ...
    companion object {
        private val TAG: String = ProgressWheel::class.java.simpleName
    }
}
