...
class CloseGuardDeliveryListActivity : CustomActivity() {
    ...
    private var irisInsDataArrayList: ArrayList<ClosedGuardDeliveryListResponse.Datum>? =
        ArrayList()
    private val irisInsDataArrayListBlock = ArrayList<ClosedGuardDeliveryListResponse.Datum?>()
    private var closedGuardDeliveryListAdapter: ClosedGuardDeliveryListAdapter? = null
    private val block_List: MutableList<String?> = ArrayList()
    private var closedGuardDeliveryListResponse: ClosedGuardDeliveryListResponse? =
        ClosedGuardDeliveryListResponse()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivityCloseGuardDeliveryListBinding.inflate(
            layoutInflater
        )
    ...
    private fun setUpData() {
        if (prefManager!!.useR_TYPE_ID == "1" && prefManager!!.useR_TYPE
            ...
            binding!!.rlUserList.visibility = View.VISIBLE
            ...
            binding!!.rlUserList.visibility = View.GONE
            binding!!.spacer.visibility = View.VISIBLE
...
//        });
    ...
    private fun getCloseGuardDeliveryList(
        ...
        if (Constants.isNetworkAvailable(mActivity!!)) {
            ...
            val call = apiInterface.getCloseGuardDeliveryList(
                USER_Id, fpscodee, districtId,
                seUserId, fromDate, toDate, "1", "" + currentPage, "100"
            )
            call.enqueue(object : Callback<ClosedGuardDeliveryListResponse?> {
                override fun onResponse(
                    call: Call<ClosedGuardDeliveryListResponse?>,
                    response: Response<ClosedGuardDeliveryListResponse?>
                ) {
                    hideProgress()
                        ...
                        if (response.code() == 200) {
                            if (response.body() != null) {
                                ...
                                if (response.body()!!.status == "200") {
                                    ...
                                    val message = closedGuardDeliveryListResponse!!.message

                                    //  Toast.makeText(InstallationPendingList.this, message, Toast.LENGTH_SHORT).show();
                                    allPagesLoaded =
                                        if (closedGuardDeliveryListResponse!!.currentPage == closedGuardDeliveryListResponse!!.totalPages) true
                                        else false

                                    irisInsDataArrayList!!.clear()
                                    //    irisInsDataArrayListBlock.clear();
                                    irisInsDataArrayList = closedGuardDeliveryListResponse!!.data
                                    ...
                                    Log.d(
                                        "irisInsDataArrayListBlock",
                                        "Size...." + irisInsDataArrayListBlock.size
                                    )
                                    ...
                                    binding!!.txtTotalPages.text =
                                        "Total Pages: " + closedGuardDeliveryListResponse!!.totalPages
                                    binding!!.txtTotalRecord.text =
                                        "Total Records: " + closedGuardDeliveryListResponse!!.totalItems
...
//                                    binding.rvDelivered.setAdapter(closedGuardDeliveryListAdapter);
                                    ...
                                    for (i in irisInsDataArrayList!!.indices) {
                                        if (!block_List.contains(irisInsDataArrayList!!.get(i)!!.blockName)) {
                                            block_List.add(irisInsDataArrayList!!.get(i)!!.blockName)
                                        }
                                    }
    ...
    private fun ExcelformTable() {
        var excelData = ArrayList<ClosedGuardDeliveryListResponse.Datum>()
        ...
        val type = object : TypeToken<ArrayList<ClosedGuardDeliveryListResponse.Datum?>?>() {
        }.type
...
//        }
        ...
        val firstSheet = workbook.createSheet("Closed Guard Delivered$corStartDate - $corEndDate")
...
*/
        ...
        if (excelData != null && excelData.size > 0) {
            for (i in excelData.indices) {
                ...
                val Date = detailsInfo.closeGuardDeliverdOnStr.toString()
                val Address = detailsInfo.closeGuardDeliveryAddress.toString()
