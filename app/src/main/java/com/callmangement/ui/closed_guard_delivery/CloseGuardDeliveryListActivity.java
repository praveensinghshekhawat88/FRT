...
public class CloseGuardDeliveryListActivity extends CustomActivity {
    private ActivityCloseGuardDeliveryListBinding binding;
    ...
    private ArrayList<ClosedGuardDeliveryListResponse.Datum> irisInsDataArrayList = new ArrayList<>();
    private ArrayList<ClosedGuardDeliveryListResponse.Datum> irisInsDataArrayListBlock = new ArrayList<>();
    private ClosedGuardDeliveryListAdapter closedGuardDeliveryListAdapter;
    private final List<String> block_List = new ArrayList<>();
    private ClosedGuardDeliveryListResponse closedGuardDeliveryListResponse = new ClosedGuardDeliveryListResponse();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        binding = ActivityCloseGuardDeliveryListBinding.inflate(getLayoutInflater());
    ...
    private void init() {
        ...
        binding.actionBar.textToolbarTitle.setText(R.string.closed_guard_delivery_list);
    ...
    private void setUpData() {
        if (prefManager.getUSER_TYPE_ID().equals("1") && prefManager.getUSER_TYPE()
            ...
            binding.rlUserList.setVisibility(View.VISIBLE);
            ...
            binding.rlUserList.setVisibility(View.GONE);
            binding.spacer.setVisibility(View.VISIBLE);
...
//        });
    ...
    private void getCloseGuardDeliveryList(String districtId, String fromDate, String toDate, String fpscodee,
                                           String serialno) {
        ...
        if (Constants.isNetworkAvailable(mActivity)) {
            ...
            Call<ClosedGuardDeliveryListResponse> call = apiInterface.getCloseGuardDeliveryList(USER_Id, fpscodee, districtId,
                    seUserId, fromDate, toDate, "1", "" + currentPage, "100");
            call.enqueue(new Callback<ClosedGuardDeliveryListResponse>() {
                @Override
                public void onResponse(@NonNull Call<ClosedGuardDeliveryListResponse> call,
                                       @NonNull Response<ClosedGuardDeliveryListResponse> response) {
                    hideProgress();
                        ...
                        if (response.code() == 200) {
                            if (response.body() != null) {
                                ...
                                if (Objects.requireNonNull(response.body()).getStatus().equals("200")) {
                                    closedGuardDeliveryListResponse = response.body();
                                    String message = closedGuardDeliveryListResponse.getMessage();
                                    //  Toast.makeText(InstallationPendingList.this, message, Toast.LENGTH_SHORT).show();

                                    if (closedGuardDeliveryListResponse.getCurrentPage().equals(closedGuardDeliveryListResponse.getTotalPages()))
                                        allPagesLoaded = true;
                                    else
                                        allPagesLoaded = false;
                                    ...
                                    //    irisInsDataArrayListBlock.clear();
                                    irisInsDataArrayList = closedGuardDeliveryListResponse.getData();
                                    Collections.reverse(irisInsDataArrayList);
                                    irisInsDataArrayListBlock.addAll(irisInsDataArrayList);
                                    closedGuardDeliveryListAdapter.notifyDataSetChanged();
                                    Log.d("irisInsDataArrayList", "Size...." + irisInsDataArrayList.size());
                                    Log.d("irisInsDataArrayListBlock", "Size...." + irisInsDataArrayListBlock.size());


                                    binding.rvDelivered.setVisibility(View.VISIBLE);
                                    //    binding.rlBlock.setVisibility(View.VISIBLE);

                                    binding.txtTotalPages.setText("Total Pages: " + closedGuardDeliveryListResponse.getTotalPages());
                                    binding.txtTotalRecord.setText("Total Records: " + closedGuardDeliveryListResponse.getTotalItems());
                                    binding.txtPageNumber.setText("Page Number: " + closedGuardDeliveryListResponse.getCurrentPage());
...
//                                    binding.rvDelivered.setAdapter(closedGuardDeliveryListAdapter);
                ...
                @Override
                public void onFailure(@NonNull Call<ClosedGuardDeliveryListResponse> call, @NonNull Throwable error) {
                    hideProgress();
                    makeToast(getResources().getString(R.string.error));
                    call.cancel();
    ...
    private void ExcelformTable() {
        ...
        ArrayList<ClosedGuardDeliveryListResponse.Datum> excelData = new ArrayList<>();
        ...
        Type type = new TypeToken<ArrayList<ClosedGuardDeliveryListResponse.Datum>>() {
        }.getType();
...
//        }
        ...
        HSSFSheet firstSheet = workbook.createSheet("Closed Guard Delivered" + corStartDate + " - " + corEndDate);
...
*/
        ...
        Log.d("mylist", " -------------- " + excelData.size());
        if (excelData != null && excelData.size() > 0) {
            for (int i = 0; i < excelData.size(); i++) {
                ClosedGuardDeliveryListResponse.Datum detailsInfo = excelData.get(i);
                ...
                String Date = String.valueOf(detailsInfo.getCloseGuardDeliverdOnStr());
                String Address = String.valueOf(detailsInfo.getCloseGuardDeliveryAddress());
