...
//        binding.pieChart.setEntryLabelTextSize(12f);
    ...
    private fun fetchComplaintList(districtId: String?) {
        val fromDate = binding!!.textFromDate.text.toString().trim { it <= ' ' }
        val toDate = binding!!.textToDate.text.toString().trim { it <= ' ' }
        //isLoading();
    ...
    private fun tehsilList(user_id: String?) {
        viewModel!!.getTehsil(user_id).observe(
            this,
            Observer<ModelTehsil?> { modelTehsil: ModelTehsil? ->
    ...
    private fun updateLabelFromDate() {
        ...
        prefManager!!.froM_DATE =
            binding!!.textFromDate.text?.toString()?.trim { it <= ' ' }
    }
    ...
    private fun updateLabelToDate() {
        ...
        prefManager!!.tO_DATE =
            binding!!.textToDate.text?.toString()?.trim { it <= ' ' }
    ...
    private fun turnOnGps() {
        GpsUtils(mContext!!).turnGPSOn(GpsUtils.onGpsListener { isGPSEnable: Boolean ->
            isGPS = isGPSEnable
        })
    }
    ...
    companion object {
        ...
        fun rgb(hex: String): Int {
            val color = hex.replace("#", "").toLong(16) // This gives us a Long value
            val r = ((color shr 16) and 0xFF).toInt() // Extract red value
            val g = ((color shr 8) and 0xFF).toInt()  // Extract green value
            val b = (color and 0xFF).toInt()          // Extract blue value
            return Color.rgb(r, g, b) // Return the color as an Int using the Color.rgb method
        }
    }
}