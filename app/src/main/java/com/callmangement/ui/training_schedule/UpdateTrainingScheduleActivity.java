...
public class UpdateTrainingScheduleActivity extends CustomActivity implements View.OnClickListener {
    ...
    private void setUpData(){
        ...
        binding.inputTrainingNumber.setText(model.trainingNo);
    ...
    private void updateTrainingSchedule(){
        ...
        if (districtNameEng.equalsIgnoreCase("--" + getResources().getString(R.string.district) + "--")){
            ...
            isLoading();
            viewModel.updateTraining(prefManager.getUSER_Id(), districtId, tehsilId, model.trainingId, startDateTime, endDateTime, model.trainingNo, Constants.convertStringToUTF8(address), Constants.convertStringToUTF8(description), Constants.convertStringToUTF8(title)).observe(this, modelCreateTrainingSchedule -> {
                isLoading();
            ...
            if (modelDistrict.status.equals("200")){
                ...
                if (district_List != null && district_List.size() > 0) {
                    ...
                    for (int i = 0 ; i < district_List.size(); i++){
                        if (district_List.get(i).getDistrictId().equals(model.districtID)) {
                        ...
                        if (Objects.requireNonNull(modelTehsil).status.equals("200")) {
                            ...
                            if (tehsil_list != null && tehsil_list.size() > 0) {
                                ...
                                for (int i = 0 ; i < tehsil_list.size(); i++){
                                    if (tehsil_list.get(i).getTehsilId().equals(model.tehsilID)) {
                                        tehsilId = tehsil_list.get(i).getTehsilId();
                                        tehsilNameEng = tehsil_list.get(i).tehsilNameEng;
