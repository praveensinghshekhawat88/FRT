...
class BiometricDeliveryListActivity() : CustomActivity() {
    ...
    private fun setUpData() {
        ...
        binding!!.rvDelivered.setLayoutManager(LinearLayoutManager(mContext))
    ...
    private fun getSensorDistributionList(districtId: String?, fromDate: String?, toDate: String?) {
        if (isNetworkAvailable((mActivity)!!)) {
            ...
            val call: Call<SensorDistributionDetailsListResp?>? =
                apiInterface.getSensorDistributionDetailsList(
                        ...
                        if (response.code() == 200) {
                            if (response.body() != null) {
                                ...
                                if ((Objects.requireNonNull(response.body())
                                    ...
                                    val sensorSummaryResponse: SensorDistributionDetailsListResp? =
                                        response.body()
                                    val message: String = sensorSummaryResponse!!.message!!

                                    if (sensorSummaryResponse.currentPage == sensorSummaryResponse.totalPages) allPagesLoaded =
                                        true
                                    else allPagesLoaded = false
