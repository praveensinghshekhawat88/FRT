...
import com.callmangement.network.MultipartRequester.fromFile
...
class ComplaintRepository(private val mContext: Context) {
    ...
    val modelDistrictMutableLiveData = MutableLiveData<ModelDistrict?>()
...
//                        }
    ...
    fun getModelResolveComplaintMutableLiveData(
        ...
        if (DSO_LETTER_TYPE == "IMAGE") {
            dso_body = fromFile("DamageApprovalLetter", damageApprovalLetter!!)
        ...
        val call = service.resolveComplaint(
            fromString(user_id),
            fromString(IsPhysicalDamage),
            fromString(remark),
            fromString(complaint_reg_no),
            fromString(getStatus),
            fromString(replacePartName),
            fromString(caurierDetails),
            fromString(challanNo),
            fromString(seRemarkDate),
            fromString(replacePartsIds),
            fromFile("se_image", se_image!!),
            dso_body
        )
    ...
    fun getModelChallanUploadMutableLiveData(
        ...
        val call = service.challanUpload(
            fromString(userId),
            fromString(ComplainId),
            fromString(ComplainRegNo),
            fromString(ChallanNo),
            fromFile("chllanImage", chllanImage!!)
        )
        ...
        call!!.enqueue(object : Callback<ModelTehsil?> {
            override fun onResponse(call: Call<ModelTehsil?>, response: Response<ModelTehsil?>) {
                if (response.isSuccessful) {
                    ...
                    if (Objects.requireNonNull(modelTehsil)!!.status == "200") {
                        val modelTehsilList: List<ModelTehsilList?>? = modelTehsil!!.tehsil_List
