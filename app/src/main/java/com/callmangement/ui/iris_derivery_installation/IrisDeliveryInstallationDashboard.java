...
public class IrisDeliveryInstallationDashboard extends CustomActivity {
    private ActivityIrisDeliveryInstallationBinding binding;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        binding = ActivityIrisDeliveryInstallationBinding.inflate(getLayoutInflater());
    ...
    private void init() {
        ...
        binding.actionBar.textToolbarTitle.setText("Iris Dashboard");
    ...
    private void setClickListener() {
        ...
        binding.lytClosedGuardDeliveryList.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Vibrator vibrator = (Vibrator) getSystemService(Context.VIBRATOR_SERVICE);
                // Check if the device supports vibration
                if (vibrator.hasVibrator()) {
                    ...
                    String selectedDate = String.valueOf(binding.spinner.getSelectedItemPosition());
                    String selectedDis = String.valueOf(binding.spinnerDistrict.getSelectedItemPosition());
                    Intent i = new Intent(mContext, CloseGuardDeliveryListActivity.class);
                    i.putExtra("key_districtId", districtId);
                    i.putExtra("key_fromDate", fromDate);
                    i.putExtra("key_toDate", toDate);
                    i.putExtra("key_selectedDate", selectedDate);
        ...
        binding.lytIrisInstallationPending.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Vibrator vibrator = (Vibrator) getSystemService(Context.VIBRATOR_SERVICE);
                // Check if the device supports vibration
            ...
            binding.cvInstallationPending.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    if (Delivered.equals("0")) {
                        ...
                        Intent i = new Intent(mContext, IrisNewDeliveryActivity.class);
                        i.putExtra("key_districtId", districtId);
                        i.putExtra("key_fromDate", fromDate);
                        i.putExtra("key_toDate", toDate);
                        i.putExtra("key_selectedDate", selectedDate);
    ...
    private void getInstallationCntApi(String districtId, String fromDate, String toDate) {
        ...
        if (Constants.isNetworkAvailable(mActivity)) {
            ...
            call.enqueue(new Callback<IrisDashboardResponse>() {
                @Override
                public void onResponse(@NonNull Call<IrisDashboardResponse> call, @NonNull Response<IrisDashboardResponse> response) {
                            ...
                            if (response.body() != null) {
                                if (response.body().getStatus().equals("200")) {
                                    ...
                                    if (countDatum != null && !countDatum.isEmpty()) {
                                        ...
                                        for (IrisDashboardResponse.Data datum : countDatum) {
                                            // Access individual data items in the countDatum ArrayList
                                            // Example: Accessing a property of CountDatum
                                            String total = String.valueOf(datum.getTotalIRISDispatched());
                                            Delivered = String.valueOf(datum.getTotalIRISDeliveryPending());
                ...
                @Override
                public void onFailure(@NonNull Call<IrisDashboardResponse> call, @NonNull Throwable error) {
                    //  Utils.hideCustomProgressDialogCommonForAll();
                    hideProgress();
