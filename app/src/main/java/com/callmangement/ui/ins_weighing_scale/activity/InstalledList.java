...
public class InstalledList extends CustomActivity {
    private ActivityInstalledlistBinding binding;
    ...
    ArrayList<InstalledDatum> irisInsDataArrayList;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        binding = ActivityInstalledlistBinding.inflate(getLayoutInflater());
    ...
    private void setClickListener() {
        binding.actionBar.ivBack.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                Intent i = new Intent(InstalledList.this, WeighingScaleDashboard.class);
                startActivity(i);
            }
        });
    ...
    private void getIrisWeighInstallation(String districtId, String fromDate, String toDate, String fpscodee) {
        if (Constants.isNetworkAvailable(mActivity)) {
            ...
            Call<InstalledRoot> call = apiInterface.IrisWeighInstallation(USER_Id, fpscodee, districtId, "", "0", fromDate, toDate, "");

            call.enqueue(new Callback<InstalledRoot>() {
                @Override
                public void onResponse(@NonNull Call<InstalledRoot> call, @NonNull Response<InstalledRoot> response) {
                  hideProgress();
                        ...
                        if (response.code() == 200) {
                            if (response.body() != null) {
                                ...
                                if (Objects.requireNonNull(response.body()).getStatus().equals("200")) {
                                    ...
                                    Log.d("weighInsDataArrayList", "Size...." + irisInsDataArrayList.size());
                                    mRecyclerView.setLayoutManager(new LinearLayoutManager(InstalledList.this));
                                    mRecyclerView.addItemDecoration(new DividerItemDecoration(InstalledList.this, DividerItemDecoration.VERTICAL));
                                    mRecyclerView.setVisibility(View.VISIBLE);
                                    InstalledAdapter deliveredWgtInsAdapter = new InstalledAdapter(irisInsDataArrayList, mContext);
                ...
                @Override
                public void onFailure(@NonNull Call<InstalledRoot> call, @NonNull Throwable error) {
    ...
    private void ExcelformTable() {
        ...
        String json = sharedPreferences.getString("IL_courses", null);

        // below line is to get the type of our array list.
        Type type = new TypeToken<ArrayList<InstalledDatum>>() {}.getType();
...
*/
        ...
        if(irisInsDataArrayList != null && irisInsDataArrayList.size() > 0) {
            for (int i = 0; i < irisInsDataArrayList.size(); i++) {
                InstalledDatum detailsInfo = irisInsDataArrayList.get(i);
        ...
        try {
            ...
            Toast.makeText(InstalledList.this, "Excel Sheet Download ", Toast.LENGTH_SHORT).show();
        }
