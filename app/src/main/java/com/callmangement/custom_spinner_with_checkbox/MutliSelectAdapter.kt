...
public class MutliSelectAdapter extends RecyclerView.Adapter<MutliSelectAdapter.MultiSelectDialogViewHolder> {
    ...
    private List<ModelPartsList> mDataSet = new ArrayList<>();
    private String mSearchQuery = "";
    private Context mContext;

    MutliSelectAdapter(List<ModelPartsList> dataSet, Context context) {
    ...
    public void onBindViewHolder(final MultiSelectDialogViewHolder holder, int position) {
        ...
        if (!mSearchQuery.equals("") && mSearchQuery.length() > 1) {
            ...
            holder.dialog_name_item.setText(mDataSet.get(position).getItemName());
        }

        if (mDataSet.get(position).isSelectFlag()) {
            if (!MultiSelectDialog.selectedIdsForCallback.contains(mDataSet.get(position).getItemId())) {
                MultiSelectDialog.selectedIdsForCallback.add(mDataSet.get(position).getItemId());
            }
        }

        if (checkForSelection(mDataSet.get(position).getItemId())) {
        ...
        holder.main_container.setOnClickListener(view -> {
            if (!holder.dialog_item_checkbox.isChecked()) {
                MultiSelectDialog.selectedIdsForCallback.add(mDataSet.get(holder.getAdapterPosition()).getItemId());
                holder.dialog_item_checkbox.setChecked(true);
                mDataSet.get(holder.getAdapterPosition()).setSelectFlag(true);
                notifyItemChanged(holder.getAdapterPosition());
            } else {
                removeFromSelection(mDataSet.get(holder.getAdapterPosition()).getItemId());
                holder.dialog_item_checkbox.setChecked(false);
                mDataSet.get(holder.getAdapterPosition()).setSelectFlag(false);
                notifyItemChanged(holder.getAdapterPosition());
            }
        });
    }
    ...
    private void setHighlightedText(int position, TextView textview) {
        String name = mDataSet.get(position).getItemName();
        SpannableString str = new SpannableString(name);
        int endLength = name.toLowerCase().indexOf(mSearchQuery) + mSearchQuery.length();
        ColorStateList highlightedColor = new ColorStateList(new int[][]{new int[]{}}, new int[]{ContextCompat.getColor(mContext, R.color.colorOrange)});
    ...
    void setData(List<ModelPartsList> data, String query, MutliSelectAdapter mutliSelectAdapter) {
        this.mDataSet = data;
